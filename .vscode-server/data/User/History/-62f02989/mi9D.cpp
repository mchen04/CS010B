...
void reorder(vector<double> &flight_path_angle, vector<double> &lift_coefficient){
    ...
    for (unsigned int i = 0; i < flight_path_angle.size(); i++){
        for (unsigned int j = 0; j < flight_path_angle.size() - i; j++){
            if (flight_path_angle.at(i) > flight_path_angle.at(i + 1 )){
                swap(flight_path_angle.at(i), flight_path_angle.at(i+1)) ;
                swap(lift_coefficient.at(i), lift_coefficient.at(i+1)) ;
            }
        }
    }
}
