...
void ShoppingCart::printTotal() const {
    ...
    if ( numItemsInCart == 0 ){
        ...
        for ( unsigned i = 0; i < numItemsInCart(); i++ ){
            cout << ShoppingCart.at(i).name() << " " << cout << ShoppingCart.at(i).quantity() << " @ $" << cout << ShoppingCart.at(i).price() << " = $" << (cout << ShoppingCart.at(i).quantity() * ItemToPurchase.price()) << endl ;
        }
    }
...
void ShoppingCart::printDescription() const {
    ...
    for ( unsigned i = 0; i < numItemsInCart; i++ ){
        cout << ItemToPurchase.name() << ": " << ItemToPurchase.description() << endl ;
    }
}
