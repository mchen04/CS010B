...
SortedSet::SortedSet(const SortedSet&) : IntList(copy) {}
...
const SortedSet SortedSet::operator|(const SortedSet& secondSet){
    for (auto temp = secondSet.head; temp != nullptr; temp = temp -> next){
        IntList::push_back(secondSet -> value) ;
    }
    remove_duplicates() ;
    selection_sort() ;
    return *this ;
...
const SortedSet operator&(const SortedSet& secondSet){
    for (auto temp = secondSet.head; temp != nullptr; temp = temp -> next){
        IntList::push_back(secondSet -> value) ;
    }
    selection_sort() ;
